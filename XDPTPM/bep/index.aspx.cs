            ...
            using(SqlCommand cmd = new SqlCommand(query, connection.conn))
            {
                using (SqlDataReader dataReader = cmd.ExecuteReader())
                {
                    while (dataReader.Read())
                    {
                        ...
                        string itemHtml = $"<li class=\"item\"><a href=\"orderDetails.aspx?orderDetailsID={orderDetailsID}&tableID={tableID}&orderID={ID}\"><img src=\"../assets/bep/assets/img/images (1).png\"/><p>Bàn số {tableID}</p></a></li>";
